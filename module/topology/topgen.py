"""
Optimized Topology Generation Module
"""
...
class TopologyGenerator:
    """Optimized topology generator with caching"""
...
"""
    ...
    def _collect_output_files(self, working_dir: Path) -> Dict[str, List[str]]:
        """Collect output files using generators"""
        return {
            'topology_files': [str(f) for f in working_dir.rglob('*.itp')],
            'gro_files': [str(f) for f in working_dir.rglob('*.gro')],
            'top_files': [str(f) for f in working_dir.rglob('*.top')],
            'frcmod_files': [str(f) for f in working_dir.rglob('*.frcmod')],
            'mol2_files': [str(f) for f in working_dir.rglob('*.mol2') if 'gaff2' in f.name]
        }

    def _add_hydrogens_to_ligand(self, ligand_pdb_path: Path, verbose: bool = False) -> bool:
        """Add hydrogens to ligand using obabel and overwrite original file"""
        ...
        try:
            result = subprocess.run(
                [self._obabel_path, str(ligand_pdb_path), '-O', str(tmp_path), '-h'],
                capture_output=True,
                text=True,
                timeout=120
            )
    ...
    @error_handler
    @timing_decorator
    def generate_topology(self, ligand_pdb_path: Path, output_dir: Optional[Path] = None, 
                         charge: int = 0, verbose: bool = False) -> Dict:
        """Generate topology with optimized file operations"""
        ligand_pdb_path = Path(ligand_pdb_path)
        ...
        if working_dir.exists():
            ...
            if frcmod_files and mol2_files:
                ...
                return {
                    ...
                    'input_file': str(ligand_pdb_path),
        ...
        try:
            ...
            # Generate leap input
            self._generate_leap_input(
                ligand_pdb_path.parent, working_dir,
                [Path(f) for f in file_dict['frcmod_files']],
                [Path(f) for f in file_dict['mol2_files']],
                verbose
            )
    ...
    @timing_decorator
    def _generate_leap_input(self, parent_dir: Path, acpype_dir: Path, 
                            frcmod_files: List[Path], mol2_files: List[Path], 
                            verbose: bool = False) -> bool:
        """Generate leap.in file using template"""
        parent_dir = Path(parent_dir)
        acpype_dir = Path(acpype_dir)
        
        # Find protein PDB using generator
        ...
        # Format template efficiently
        leap_content = self.leap_template.format(
            protein_pdb=protein_rel,
            frcmod=frcmod_rel,
            mol2=mol2_rel
        )
        ...
        leap_sys2_content = self.leap_sys2_template.format(
            protein_pdb=protein_rel,
            frcmod=frcmod_rel
        )
    ...
    def process_batch(self, ligand_pdb_files, output_dir=None, charge=0, verbose=False):
        results = []
        ...
        for ligand_file in ligand_pdb_files:
            ...
            result = self.generate_topology(ligand_path, output_dir, charge, verbose)
    ...
    def _generate_topology_wrapper(self, args):
        ligand_pdb, charge, verbose = args
        ligand_name = ligand_pdb.stem.replace('_ligand', '')
        output_subdir = ligand_pdb.parent
        return self.generate_topology(ligand_pdb, output_subdir, charge=charge, verbose=verbose)
    
    def _ligand_files_generator(self, output_dir: Path) -> Generator[Path, None, None]:
        """Generator to yield ligand PDB files"""
        yield from output_dir.rglob('*_ligand.pdb')
    
    @timing_decorator
    def process_from_directory(self, output_dir: Path, charge: int = 0, 
                              verbose: bool = False, parallel: bool = False, 
                              n_processes: Optional[int] = None) -> List[Dict]:
        """Process all ligand files from directory with optional parallelization"""
        ...
        if parallel and len(ligand_files) > 1:
            ...
            args_list = [(ligand_pdb, charge, verbose) for ligand_pdb in ligand_files]
            ...
            # Sequential processing using map
            return list(map(
                lambda ligand_pdb: self.generate_topology(
                    ligand_pdb, ligand_pdb.parent, charge=charge, verbose=verbose
                ),
                ligand_files
            ))
