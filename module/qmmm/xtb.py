"""
Optimized XTB Processing Module
"""
...
class XTBProcessor:
    ...
    def run_tleap(self, leap_input_path):
        ...
        try:
            ...
            if not gas_complex_pdb.exists():
                return {
                    'status': 'error',
                    'error_message': 'gas.complex.pdb not generated',
            ...
            return {
                'status': 'success',
                'gas_complex_pdb': str(gas_complex_pdb),
                'output_dir': str(work_dir),
                'leap_sys2_status': leap_sys2_status
            }
        except RuntimeError as err:
            return {
                'status': 'error',
    ...
    def _parse_pdb_atoms(self, pdb_path: Path) -> Tuple[List[Tuple], List[Dict], Dict]:
        """Generator-based PDB parsing for memory efficiency"""
        ligand_coords = []
        protein_atoms = []
        residue_info = {}
    ...
    @timing_decorator
    def get_nearby_residues(self, pdb_path: Path, ligand_resname: str = 'LIG', 
                          distance_cutoff: float = 2.5, verbose: bool = False) -> List[Tuple[int, str]]:
        """Find nearby residues using optimized distance calculation"""
        ...
        try:
            ...
            cutoff_sq = distance_cutoff ** 2  # Avoid sqrt in distance calculation
    ...
    def calculate_qm_charge(self, residues_with_names, ligand_charge=0):
    ...
    def run_sander_min(self, work_dir):
        ...
        for req_file in required_files:
            if not (work_dir / req_file).exists():
                return {
                    'status': 'error',
                    'error_message': f'Required file {req_file} not found',
                    'output_dir': str(work_dir)
                }
        ...
        for idx, spin_value in enumerate(spin_attempts):
            ...
            try:
                result = subprocess.run(
                    [
                        ...
                        '-i', 'min.mdin',
                        '-o', '01.min.mdout',
                        '-p', 'wet.complex.prmtop',
                        '-c', 'wet.complex.rst7',
                        '-ref', 'wet.complex.rst7',
                        '-x', '01.min.trj',
                        '-inf', '01.min.mdinfo',
                        '-r', '01.min.rst7'
                    ],
            ...
            if result.returncode == 0 and mdout_file.exists() and rst7_file.exists():
                return {
                    'status': 'success',
                    'mdout': str(mdout_file),
                    'rst7': str(rst7_file),
                    'output_dir': str(work_dir),
                    'spin_used': spin_value
                }
    ...
    def _convert_ligand_pdb_to_xyz(self, ligand_pdb: Path, xyz_path: Path, verbose: bool = False) -> Dict:
        ...
        if not self._obabel_path:
            return {
                'status': 'error',
                'error_message': 'obabel executable not found in PATH'
            }
        try:
            result = subprocess.run(
                [self._obabel_path, str(ligand_pdb), '-O', str(xyz_path)],
                capture_output=True,
                text=True,
                timeout=120
            )
            ...
            return {
                'status': 'error',
                'error_message': 'obabel conversion timed out'
            }
        except Exception as exc:
            return {
                'status': 'error',
                'error_message': f'obabel conversion failed: {exc}'
            }
        ...
        return {
            'status': 'error',
            'error_message': 'obabel conversion returned non-zero exit status',
            'stdout': result.stdout,
            'stderr': result.stderr
        }

    def run_xtb_ligand(self, work_dir, ligand_charge=0, verbose: bool = False):
        work_dir = Path(work_dir)
        ligand_pdb = next(iter(sorted(work_dir.glob('*_ligand.pdb'))), None)
        if not ligand_pdb or not ligand_pdb.exists():
            return {
                'status': 'error',
                'error_message': 'Ligand PDB file not found for XTB ligand calculation'
            }
        ...
        for uhf in uhf_attempts:
            try:
                result = subprocess.run(
                    ['xtb', 'qmmm.xyz', '-chrg', str(ligand_charge), '-uhf', str(uhf)],
                    cwd=str(work_dir),
                    capture_output=True,
                    text=True,
                    timeout=600
    ...
    def run_xtb_qm(self, work_dir, qm_charge=0):
        ...
        if not qmmm_pdb.exists():
            return {
                'status': 'error',
                'error_message': 'qmmm_region.pdb not found',
                'output_dir': str(work_dir)
            }
        ...
        if not self.pdb_to_xyz(qmmm_pdb, qmmm_xyz):
            return {
                'status': 'error',
                'error_message': 'Failed to convert PDB to XYZ',
                'output_dir': str(work_dir)
            }
        
        # Try uhf values 1 and 2
        for uhf_value in [1, 2]:
            try:
                result = subprocess.run(
                    ['xtb', 'qmmm_region.xyz', '-chrg', str(qm_charge), '-uhf', str(uhf_value)],
                    cwd=str(work_dir),
                    capture_output=True,
                    text=True,
                    timeout=600
                ...
                # Check if xtb ran successfully (return code 0)
                if result.returncode == 0:
                    ...
                    if uhf_value == 2:
                        ...
                        return {
                            'status': 'error',
                            'error_message': f'XTB failed with both uhf=1 and uhf=2',
                            'output_dir': str(work_dir),
                            'stdout': result.stdout,
                            'stderr': result.stderr
                        }
                    # Continue to next uhf value
                    
            except subprocess.TimeoutExpired:
                if uhf_value == 2:
                    return {
                        'status': 'error',
                        'error_message': 'XTB calculation timed out',
                        'output_dir': str(work_dir)
                    }
                # Continue to next uhf value
            except Exception as e:
        ...
        return {
            'status': 'error',
            'error_message': 'XTB calculation failed unexpectedly',
            'output_dir': str(work_dir)
        }
    ...
    def _run_sander_wrapper(self, args):
        ...
        if sys2_min_result['status'] != 'success':
            return {
                ...
                'sp_result': sp_result,
                'vdwm_result': vdwm_result,
                'sys2_min_result': sys2_min_result,
                'output_dir': str(work_dir),
                'ligand_xtb_result': None
            }
        ...
        ligand_xtb_result = self.run_xtb_ligand(work_dir, ligand_charge=qm_charge, verbose=verbose)
        if ligand_xtb_result['status'] != 'success':
            return {
                ...
                'sys2_min_result': sys2_min_result,
                'sys2_sp_result': sys2_sp_result,
                'ligand_xtb_result': ligand_xtb_result,
                'output_dir': str(work_dir)
            }
        ...
        return {
            'status': 'success' if xtb_result['status'] == 'success' else 'error',
            'min_result': min_result,
            'sp_result': sp_result,
            'vdwm_result': vdwm_result,
            'sys2_min_result': sys2_min_result,
    ...
    def run_sander_batch(self, output_dir, qm_charge=0, verbose=False, parallel=True, n_processes=None):
        ...
        if not subdirs:
            return {
                'status': 'error',
                'error_message': 'No subdirectories with required files found'
            }
    ...
    def update_mdin_with_qmmask(self, mdin_path: Path, qmmask: str, qmcharge: int = 0) -> bool:
        """Update MDIN file with QM mask using efficient string operations"""
        ...
        try:
            ...
            # Use list comprehension for efficient line processing
            updated_lines = [
                f" qmmask = ':{qmmask}',\n" if 'qmmask' in line and '=' in line
                else f" qmcharge = {qmcharge},\n" if 'qmcharge' in line and '=' in line
                else line
                for line in lines
            ]
    ...
    def process_directory(self, output_dir, distance_cutoff=None, ligand_charge=0, verbose=False, parallel=True, n_processes=None):
        output_dir = Path(output_dir)
        ...
        if not subdirs:
            return {
                'status': 'error',
                'error_message': 'No subdirectories with leap.in found'
            }
        
        # If distance_cutoff is None, only run tleap without QM mask calculation
        if distance_cutoff is None:
            ...
            return {
                'status': 'success',
                'total_count': len(subdirs),
                'success_count': tleap_success,
                'results': results
            }
        
        # Normal processing with QM mask calculation
        if parallel:
            if n_processes is None:
                n_processes = min(cpu_count(), len(subdirs))
        ...
        for result in results:
            ...
            qm_charge_local = self.calculate_qm_charge(residues_with_ligand, ligand_charge=ligand_charge) if residues_with_ligand else 0
            if verbose:
                print(f"{result['subdir']}: Found {len(residues_with_ligand)} residues for QM mask")
            per_directory_masks.append({
                'subdir': result['subdir'],
                'residues': residues_with_ligand,
                'qmmask': qmmask_local,
                'qmmask_sys2': qmmask_sys2_local,
                'qm_charge': qm_charge_local
            })
        
        if not per_directory_masks:
            return {
                'status': 'error',
                'error_message': 'No successful processing',
                'results': results
            }
        ...
        for subdir in subdirs:
            ...
            if not subdir_data:
                continue
            mdin_files = [
                ('min.mdin', True, subdir_data['qmmask']),
                ('min_sys2.mdin', False, subdir_data['qmmask_sys2']),
                ('sp.mdin', False, subdir_data['qmmask']),
                ('sp_sys2.mdin', False, subdir_data['qmmask_sys2'])
            ]
        ...
        return {
            ...
            'total_count': len(subdirs),
            'results': results,
            'per_directory_masks': per_directory_masks
        }
    
    def _run_tleap_only(self, subdir):
        """Run tleap only without QM mask analysis."""
