...
def main():
    ...
    parser.add_argument('-i', '--input', type=str, default='samp',
                        help='Input directory containing PDB files (default: samp)')
    parser.add_argument('-o', '--output', type=str, default='output',
                        help='Output directory for processed files (default: output)')
    parser.add_argument('-t', '--topology', action='store_true',
                        help='Generate topology files for ligands using acpype')
    ...
    parser.add_argument('--xtb', action='store_true',
                        help='Run tleap, calculate QM mask, and run sander minimization')
    parser.add_argument('-d', '--distance', type=float, default=None,
                        help='Distance cutoff for finding nearby residues (in Angstroms). If not specified, qmmask and qmcharge will not be modified')
    parser.add_argument('-p', '--parallel', action='store_true',
                        help='Enable parallel processing for topology generation and tleap')
    parser.add_argument('-n', '--nproc', type=int, default=None,
                        help='Number of processes for parallel processing (default: auto-detect CPU count)')
    parser.add_argument('-v', '--verbose', action='store_true',
                        help='Enable verbose output')
    parser.add_argument('--analyze', action='store_true',
                        help='Analyze output files and generate Excel report')
    ...
    # Use parallel processing for PDB files if enabled
    ...
    if args.verbose:
        ...
        if error_count > 0:
            ...
            error_msgs = '\n'.join(
                f"  Error in {Path(r['input_file']).name}: {r['error_message']}"
    ...
    if args.topology and success_count > 0:
        ...
        topo_results = generator.process_from_directory(output_dir, charge=args.charge, verbose=args.verbose, parallel=args.parallel, n_processes=args.nproc)
        ...
        if args.verbose:
            ...
            if topo_error > 0:
                ...
                error_msgs = '\n'.join(
                    f"  Error in {r['ligand_name']}: {r['error_message']}"
                    for r in topo_results if r.get('status') == 'error'
                )
                print(error_msgs)
    ...
    if success_count > 0:
        ...
        # Use generator expression for filtering
        output_subdirs = [
            d for d in output_dir.rglob('*') 
            if d.is_dir() and (d / f"{d.name}_ligand.pdb").exists()
        ]
        ...
        if args.verbose:
            ...
            if mdin_error > 0:
                # Use join for efficient error reporting
                error_msgs = '\n'.join(
                    f"  Error in {r['output_dir']}: {r['error_message']}"
                    for r in mdin_results if r.get('status') == 'error'
                )
                print(error_msgs)
    ...
    if args.xtb and success_count > 0:
        ...
        if args.distance is not None:
            ...
            xtb_result = xtb_processor.process_directory(output_dir, distance_cutoff=args.distance, ligand_charge=args.charge, verbose=args.verbose, parallel=args.parallel, n_processes=args.nproc)
        else:
            if args.verbose:
                print(f"\nRunning tleap (QM mask calculation skipped - no distance specified)...")
            
            xtb_result = xtb_processor.process_directory(output_dir, distance_cutoff=None, ligand_charge=args.charge, verbose=args.verbose, parallel=args.parallel, n_processes=args.nproc)
        ...
        if xtb_result['status'] == 'success':
            ...
            sander_result = xtb_processor.run_sander_batch(
                output_dir,
                qm_charge=args.charge,
                verbose=args.verbose,
                parallel=args.parallel,
                n_processes=args.nproc
            )
            ...
            if sander_result['status'] == 'success':
                ...
                if sander_result['error_count'] > 0:
                    ...
                    if args.verbose:
                        # Use join for efficient error reporting
                        error_msgs = '\n'.join(
                            f"  {Path(r['output_dir']).name}: {r['error_message']}"
                            for r in sander_result['results'] if r['status'] == 'error'
                        )
                        if error_msgs:
                            print(error_msgs)
            else:
